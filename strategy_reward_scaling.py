...
# i added my reward scaling strategy here:
def calculate_reward_scaling(pole_length, base_reward=1.0):
    """
    Scale reward based on pole length difficulty.
    pole_length: current pole length from 0.4 to 1.8
    base_reward: original reward from environment (typically 1.0)
    Returns: scaled reward (which is base_reward * scaling_factor)
    """
    optimal_length = 1.0  # it's the default CartPole length
...
def train_reward_scaling_dqn(learning_rate=0.01, gamma=0.99, episodes=500,
                       buffer_capacity=10000, batch_size=32,
                       epsilon_start=1.0, epsilon_min=0.01, epsilon_decay=0.995,
                       target_update_freq=10, log_freq=25):
    ...
    for episode in range(episodes):
        ...
        while not done:
            ...
            # i apply reward scaling here as well:
            scaled_reward = calculate_reward_scaling(pole_len, reward)

            # Store transition (no reward scaling - pure baseline)
            #replay_buffer.add(state, action, reward, next_state, done)
            replay_buffer.add(state, action, scaled_reward, next_state, done) # now store transition with scaled reward
            
            state = next_state
            #episode_reward += reward
            episode_reward += scaled_reward # i changed it to track scaled rewards
...
if __name__ == "__main__":
    # Train reward scaling strategy
    plot_avg_rewards, weights, episode_rewards = train_reward_scaling_dqn(
        learning_rate=0.01,
        gamma=0.99,
        episodes=500,
    ...
    stats = compute_training_stats(episode_rewards, method_name="Reward Scaling Strategy")
