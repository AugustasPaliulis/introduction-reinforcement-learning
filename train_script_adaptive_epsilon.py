...
def deep_q_learning(learning_rate, gamma, episodes, hidden_dim):
    """
    Deep Q-Learning algorithm implementation with adaptive epsilon
    """
    ...
    epsilon_start = 1.0
    epsilon_end = 0.01
    epsilon_decay_episodes = int(episodes * 0.7)  # Decay over 70% of training
    ...
    for episode in range(episodes):
        # Adaptive epsilon decay - exponential decay that accelerates over time
        if episode < epsilon_decay_episodes:
            # Exponential decay
            epsilon = epsilon_end + (epsilon_start - epsilon_end) * \
                      np.exp(-1. * episode / (epsilon_decay_episodes * 0.4))
        else:
            # Force more exploitation in final episodes
            epsilon = epsilon_end
